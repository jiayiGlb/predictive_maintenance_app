...
def load_device_data(device_name, start_date, end_date, sensor):
    ...
    files_to_read = [os.path.join(base_path, f"{date.strftime('%Y%m%d')}.log") for date in date_range if os.path.exists(os.path.join(base_path, f"{date.strftime('%Y%m%d')}.log"))]
    ...
    for file_path in files_to_read:
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                for line in file:
                    if 'Sent Status' in line:
                        line = clean_garbled_data(line)
                        data = extract_data(file_path, line)
    ...
    full_data['timestamp'] = pd.to_datetime(full_data['timestamp'], errors='coerce')  # Ensure timestamps are valid
    full_data[sensor] = pd.to_numeric(full_data[sensor], errors='coerce')  # Ensure sensor data is numeric
    full_data.dropna(subset=['timestamp', sensor], inplace=True)  # Drop rows with invalid or missing values
...
def generate_plot_with_thresholds(data, sensor, thresholds):
    ...
    if 'low' in thresholds:
        for i, threshold in enumerate(thresholds['low']):
            fig.add_hline(
                y=threshold,
                line_color = f"rgb(255, {i * 50}, 0)",  # Gradient yellow tones
                line_width=1,
            )
